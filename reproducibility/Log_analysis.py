...
"""This code computes relevant metrics over each simulated log in the output_log folder"""

# Folders
in_folder = 'output_log'
out_folder = 'output_analysis'

# Pattern: all files like simulated_log_order_process_*.csv
pattern = os.path.join(in_folder, 'simulated_log_*.csv')
...
for file_path in glob.glob(pattern):
    ...
    # (3) relationships size stats for "Load", count how many items are in the relationships
    ...
    load_sizes = df_load["relationships"].dropna().apply(count_elements).dropna()
    ...
    metrics["Remove_Item_orders"] = df_remove["relationships"].nunique()
...
output_path = os.path.join(out_folder, "log_metrics.csv")
